void replay_shms (Int_t RunNumber = 0, Int_t MaxEvent = 0,const char* ftype="matrixopt",const char* rtype="shms") {
  ...
  const char* RunFileNamePattern = "%s_all_%05d.dat";
  ...
  gHcParms->AddString("g_ctp_database_filename", "UTIL_COMM_3PASS/DBASE/SHMS/STD/standard.database");
 ...
 THcHodoEff* peff = new THcHodoEff("phodeff"," SHMS hodo efficiency","P.hod");
  ...
  THcRun* run = new THcRun( pathList, Form(RunFileNamePattern,rtype,RunNumber) );
  ...
  // Define DEF-file
  TString DefTreeFile=Form("UTIL_COMM_3PASS/DEF-files/SHMS/%s.def",ftype);
  analyzer->SetOdefFile(DefTreeFile);
  // Define cuts file
  DefTreeFile=Form("UTIL_COMM_3PASS/DEF-files/SHMS/%s_cuts.def",ftype);
  ...
  TString TemplateFile=Form("UTIL_COMM_3PASS/TEMPLATES/SHMS/%s.template",ftype);
   analyzer->PrintReport(TemplateFile,
  			 Form("REPORT_OUTPUT/SHMS/PRODUCTION/replay_%s_%d_%d.report",ftype, RunNumber, MaxEvent));  // optional
